...
https://github.com/antoninschrab/mmdfuse-paper/blob/master/sampler_perturbations.py
...
def G(x):
    """
    Function G defined in Section 5.4 (under Eq. (17)) of our paper.
...
def f_theta(x, p, s, perturbation_multiplier=1, seed=None):
    ...
    np.random.seed(seed)
    theta = np.random.choice([-1,1], p ** d) # (p**d, ) values = 1 or -1
    output = 0
    I = list(itertools.product([i+1 for i in range(p)], repeat=d))  # set {1,...,p}^d
    for i in range(len(I)):
        j = I[i] 
        # The summation in Section 5.4 (Eq. (17)) of our paper
        output += theta[i] * np.prod([G(x[r] * p - j[r]) for r in range(d)]) 
    output *= p ** (-s) * perturbation_multiplier # c_d*P**(-s) in Eq. (17)
    if np.min(x) >= 0 and np.max(x) <= 1: # the indicator in Eq. (17)
        output += 1
    np.random.seed(None)
    return output
    

def f0(x): # check if all entries in array x are non-negative (true=1, anyfalse=0)
    """
...
def f_theta_sampler(
    ...
    inputs: f_theta_seed           : integer random seed for f_theta
            ...
            s                      : positive number (smoothness parameter of Sobolev ball (Eq. (1))
            perturbation_multiplier: positive number (c_d in Eq. (17)) 
            d                      : non-negative integer (dimension of input of density)
